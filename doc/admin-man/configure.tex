%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\label{sec:Configuring-HTCondor-Macros}Configuration Macros}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\index{HTCondor!configuration-macros}
\index{configuration: macros}

The section contains a list of the individual configuraton macros
for HTCondor.  Before attempting to set up HTCondor configuration,
you should probably read the introduction to configuration section
(~\ref{sec:Configuring-HTCondor-Intro}) and possibly the configuration
template section (~\ref{sec:Configuring-HTCondor-Templates}).
...
\Condor{startd}, not in this section. 
...
The \verb@<what-to-include>@ specification may describe a single file,
...
to cause the new values to take effect.

% It would be nice to have a note here (maybe just in a comment) as
% to what version this list is correct for...
\begin{itemize}
...
\item[\Macro{COLLECTOR\_PORT}]
  The default port used when contacting the \Condor{collector}
  and the default port the \Condor{collector} listens on if
  no port is specified.
  This variable is referenced if no port is given
  and there is no other means to find the \Condor{collector} port.
...
\item[\Macro{TMP\_DIR}]
  ...
  \Condor{config\_val} when using the \Opt{-rset} option,
  systems that use lock files when configuration variable
  \Macro{CREATE\_LOCKS\_ON\_LOCAL\_DISK} is \Expr{True},
  the Web Service API,
  and the \Condor{credd} daemon.
  There is no default value.

  If both \MacroNI{TMP\_DIR} and \MacroNI{TEMP\_DIR} are defined, 
  the value set for \MacroNI{TMP\_DIR} is used and \MacroNI{TEMP\_DIR} 
  is ignored.
...
\item[\Macro{USER\_CONFIG\_FILE}]
  ...
  Relevant only if HTCondor daemons are \emph{not} run as \Login{root}
  on Unix platforms or Local System on Windows platforms. 
  The default is \File{\$(HOME)/.condor/user\_config} on Unix platforms.
  ...
  then the Unix path \File{\$(HOME)/.condor/} prefixes the file name
...
\item[\Macro{MAIL}]
  ...
  and should use \MacroNI{SENDMAIL} instead.

\label{param:Sendmail}
\item[\Macro{SENDMAIL}]
  The full path to the \Prog{sendmail} executable.  
  If defined, which it is by default on non-Windows platforms,
  \Prog{sendmail} is used instead of the mail program defined by \MacroNI{MAIL}.
...
\item[\Macro{HISTORY\_HELPER\_MAX\_CONCURRENCY}]
  Specifies the maximum number of concurrent remote \Condor{history}
  ...
  as an invocation of \Condor{history} that specifies a \Opt{-name} option
  to query a \Condor{schedd} running on a remote machine.
...
\item[\Macro{MAX\_JOB\_QUEUE\_LOG\_ROTATIONS}]
  The \Condor{schedd} daemon periodically rotates the job queue database file,
...
\item[\Macro{CLASSAD\_USER\_PYTHON\_MODULES}]
  ...
  If module \Expr{foo} is in this list, then function \Expr{bar}
  can be invoked in ClassAds via the expression 
  \Expr{python\_invoke("foo", "bar", ...)}.
  ...
  Module writers can invoke \Expr{classad.register} at the module-level
...
\item[\Macro{CLASSAD\_USER\_PYTHON\_LIB}]
  ...
  which is needed when \Macro{CLASSAD\_USER\_PYTHON\_MODULES} is set.
  Defaults to \File{\$(LIBEXEC)/libclassad\_python\_user.so},
...
\item[\Macro{STATISTICS\_TO\_PUBLISH}]
  ...
  The special value \Expr{NONE} disables all publishing, so no statistics
  ...
  If the first aspect is \Expr{ALL},
  the option is applied to all collections.
  If the first aspect is \Expr{DEFAULT},
  the option is applied to all collections, 
  ...
  It may be \Expr{TRANSFER} to publish file transfer statistics.
  It may be \Expr{STARTER} to publish
  Statistics attributes in the ClassAd of the \Condor{starter}.
...
\end{center}
  ...
  not defined.
  If an option specifies conflicting possibilities, such as \Expr{R!R},
  ...
  attributes only for the \Condor{schedd}, and do not publish any other
  Statistics attributes:
\begin{verbatim}
  STATISTICS_TO_PUBLISH = SCHEDD:2
\end{verbatim}
  As a second example, to cause all collections other than 
  those for \Expr{DAEMONCORE} to publish at a verbosity setting of \Expr{1},
  and omit lifetime values, where the \Expr{DAEMONCORE} includes all
...
\item[\Macro{STATISTICS\_TO\_PUBLISH\_LIST}]
  ...
  that should be published in updates to the \Condor{collector} daemon,
  even though
  the verbosity specified in \MacroNI{STATISTICS\_TO\_PUBLISH} would
  ...
  specified in \MacroNI{STATISTICS\_TO\_PUBLISH}.
...
\label{param:EnableIPv4}
\item[\Macro{ENABLE\_IPV4}]
  A boolean with the additional special value of \Expr{auto}.  If true,
  HTCondor will use IPv4 if available, and fail otherwise.  If false,
  HTCondor will not use IPv4.  If \Expr{auto}, HTCondor will use IPv4
  if it can find an interface with an IPv4 address; this is the default.

\label{param:EnableIPv6}
\item[\Macro{ENABLE\_IPV6}]
  A boolean with the additional special value of \Expr{auto}.  If true,
  HTCondor will use IPv6 if available, and fail otherwise.  If false,
  HTCondor will not use IPv6.  If \Expr{auto}, HTCondor will use IPv6
  if it can find an interface with an IPv6 address; this is the default.

\label{param:PreferIPv4}
\item[\Macro{PREFER\_IPV4}]
  A boolean which will cause HTCondor to prefer IPv4 when it is able
  to choose.  HTCondor will otherwise prefer IPv6.
  The default is \Expr{True}.

\label{param:AdvertiseIPv4First}
\item[\Macro{ADVERTISE\_IPV4\_FIRST}]
  A string (treated as a boolean).  If \MacroNI{ADVERTISE\_IPV4\_FIRST}
  evaluates to \Expr{True}, HTCondor will advertise its IPv4 addresses
  before its IPv6 addresses; otherwise the IPv6 addresses will come
  first.  Defaults to \MacroUNI{PREFER\_IPV4}.

\label{param:IgnoreTargetProtocolPreference}
\item[\Macro{IGNORE\_TARGET\_PROTOCOL\_PREFERENCE}]
  A string (treated as a boolean).  If
  \MacroNI{IGNORE\_TARGET\_PROTOCOL\_PREFERENCE} evaluates to \Expr{True},
  the target's listed protocol preferences will be ignored; othwerwise
  they will not.
  Defaults to \MacroUNI{PREFER\_IPV4}.

\label{param:IgnoreDNSProtocolPreference}
\item[\Macro{IGNORE\_DNS\_PROTOCOL\_PREFERENCE}]
  A string (treated as a boolean).
  \MacroNI{IGNORE\_DNS\_PROTOCOL\_PREFERENCE} evaluates to \Expr{True},
  the protocol order returned by the DNS will be ignored; otherwise
  it will not.
  Defaults to \MacroUNI{PREFER\_IPV4}.

\label{param:PreferOutboundIPv4}
\item[\Macro{PREFER\_OUTBOUND\_IPV4}]
  A string (treated as a boolean).
  \MacroNI{PREFER\_OUTBOUND\_IPV4} evaluates to \Expr{True},
  HTCondor will prefer IPv4; otherwise it will not.
  Defaults to \MacroUNI{PREFER\_IPV4}.

\label{param:ClassAdUserMapNames}
\item[\Macro{<SUBSYS>\_CLASSAD\_USER\_MAP\_NAMES}]
  A string defining a list of names for username-to-accounting group
  mappings for the specified daemon.
  Names must be separated by spaces or commas.

\label{param:ClassAdUserMapFile}
\item[\Macro{CLASSAD\_USER\_MAPFILE\_<name>}]
  A string giving the name of a file to parse to initialize the
  map for the given username.
  Note that this macro is only used if
  \MacroNI{<SUBSYS>\_CLASSAD\_USER\_MAP\_NAMES} is defined for
  the relevant daemon.

\label{param:ClassAdUserMapData}
\item[\Macro{CLASSAD\_USER\_MAPDATA\_<name>}]
  A string containing data to be used to initialize the map for the
  given username.
  Note that this macro is only used if
  \MacroNI{<SUBSYS>\_CLASSAD\_USER\_MAP\_NAMES} is defined for
  the relevant daemon, and \Macro{CLASSAD\_USER\_MAPFILE\_<name>}
  is \emph{not} defined for the given name.

  The format for the map file and map data is the same as the
  format for the security unified map file (see
  ~\ref{sec:Security-Unified-Map-File} for details).

  The first field must be * (or a subset name - see below), the second
  field is a regex that we will match against the input, and the third
  field will be the output if the regex matches, the 3 and 4 argument
  form of the ClassAd userMap() function (see ~\ref{sec:classadFunctions})
  expect that the third field will be a comma separated
...
\Bold{Optional submaps:} If the first field of the mapfile contains
something other than *, then a submap is defined. To select a submap
for lookup, the first argument for userMap() should be "mapname.submap".
For example:
\begin{verbatim}
...
\label{param:IgnoreLeafOom}
\item[\Macro{IGNORE\_LEAF\_OOM}]
  A boolean value that, when \Expr{True},
  tells HTCondor \emph{not} to kill and hold a job that is within its
  memory allocation, even if other processes within the same cgroup
  have exceeded theirs.
  The default value is \Expr{True}.  (Note that this represents a change
  in behavior compared to versions of HTCondor older than 8.6.0;
  this configuration macro first appeared in version 8.4.11.
  To restore the previous behavior, set this value to \Expr{False}.)
...
\index{SUBSYS\_LOG macro@\texttt{<SUBSYS>\_LOG} macro}
  ...
  concatenated with \File{Log}.
  For example, the default log defined for the \Condor{master} daemon
  is \File{\$(LOG)/MasterLog}.
  The default value for other subsystems is \File{\$(LOG)/<SUBSYS>LOG}.
...
\begin{description} 
  ...
  \item{\Expr{Kb} for KiB, $2^{10}$ numbers of bytes}
  \item{\Expr{Mb} for MiB, $2^{20}$ numbers of bytes}
  \item{\Expr{Gb} for GiB, $2^{30}$ numbers of bytes}
  \item{\Expr{Tb} for TiB, $2^{40}$ numbers of bytes}
...
\item[\Macro{ENABLE\_USERLOG\_LOCKING}]
  A boolean value that defaults to \Expr{False} on Unix platforms and
  \Expr{True} on Windows platforms.
...
\index{SUBSYS\_DEBUG macro@\texttt{<SUBSYS>\_DEBUG} macro}
  ...
  \label{dflag:stats}
  \item[\Dflag{STATS}]
    \index{SUBSYS\_DEBUG macro levels@\texttt{<SUBSYS>\_DEBUG} macro levels!D\_STATS@\texttt{D\_STATS}}
    Enable debug messages relating to the TCP statistics for file
    transfers.
    Note that the shadow and starter, by default, log these statistics
    to special log files (see \Macro{SHADOW\_STATS\_LOG}
    section~\ref{param:ShadowStatsLog} and \Macro{STARTER\_STATS\_LOG},
    section~\ref{param:StarterStatsLog}).
    Note that, as of version 8.5.6, \Macro{C\_GAHP\_DEBUG} defaults
    to \MacroNI{D\_STATS}.
  ...
  \item[\Dflag{TIMESTAMP}]
    \index{SUBSYS\_DEBUG macro levels@\texttt{<SUBSYS>\_DEBUG} macro levels!D\_TIMESTAMP@\texttt{D\_TIMESTAMP}}
    This flag is different from the other flags, because it is
    used to change the formatting of all log messages that are printed,
    as opposed to specifying what kinds of messages should be printed.
    If \Dflag{TIMESTAMP} is set, the time at the beginning of each line in the
  ...
  \item[\Dflag{SUB\_SECOND}]
    \index{SUBSYS\_DEBUG macro levels@\texttt{<SUBSYS>\_DEBUG} macro levels!D\_SUB\_SECOND@\texttt{D\_SUB\_SECOND}}
    This flag is different from the other flags, because it is
    used to change the formatting of all log messages that are printed,
    as opposed to specifying what kinds of messages should be printed.
    If \Dflag{SUB\_SECOND} is set, the time at the beginning of each line in the
...
\item[\Macro{EVENT\_LOG\_ROTATION\_LOCK}]
  ...
  string parameter; its default value is \Expr{\$(LOCK)/EventLogLock}.
  If an empty value is set, then the file that is used
  is the file path of the event log itself, 
  with the string \Expr{.lock} appended.
  If \MacroNI{EVENT\_LOG} is not defined, or if
  \MacroNI{EVENT\_LOG\_MAX\_SIZE} has a value of 0 (which disables
...
\item[\Macro{SETTABLE\_ATTRS\_<PERMISSION-LEVEL>}]
  All macros that begin with \MacroNI{SETTABLE\_ATTRS} or
...
\index{configuration macro!\texttt{STARTD\_ATTRS}}
\index{configuration macro!\texttt{STARTD\_EXPRS}}
...
\MacroB{<SUBSYS>\_EXPRS}]
  ...
  \MacroNI{<SUBSYS>\_ATTRS}. It may be removed in the future, so use \MacroNI{<SUBSYS>\_ATTRS}.
  ...
  \MacroNI{SUBMIT\_ATTRS} not part of the \MacroNI{<SUBSYS>\_ATTRS}, it is
...
\label{param:MaxTimeSkip}
\item[\Macro{MAX\_TIME\_SKIP}]
  When an HTCondor daemon notices the system clock skip forwards or
  backwards more than the number of seconds specified by this parameter,
  it may take special action.  For instance, the \Condor{master} will
  restart HTCondor in the event of a clock skip.  Defaults to a value of
  1200, which in effect means that HTCondor will restart if the system clock
  jumps by more than 20 minutes.
...
\item[\Macro{SOCKET\_LISTEN\_BACKLOG}]
  ...
  which defines the backlog value for the \Procedure{listen} 
...
\item[\Macro{PIPE\_BUFFER\_MAX}]
  The maximum number of bytes read from a \Code{stdout} or \Code{stdout} pipe.
...
\item[\Macro{CCB\_POLLING\_INTERVAL}] In seconds, the smallest amount 
  ...
  as expressed with \MacroNI{CCB\_POLLING\_TIMESLICE}. 
...
\item[\Macro{CCB\_RECONNECT\_FILE}] The full path and file name of the
  ...
  open TCP connections to a file.
  Crash recovery is accomplished using the information.
  The default value is \File{\$(SPOOL)/.ccb\_reconnect}.

\label{param:CollectorUsesSharedPort}
\item[\Macro{COLLECTOR\_USES\_SHARED\_PORT}] A boolean value that
  specifies whether the \Condor{collector} uses the \Condor{shared\_port}
  daemon.  When true, the \Condor{shared\_port} will transparently
  proxy queries to the \Condor{collector} so users do not need to be aware of
  the presence of the \Condor{shared\_port} when querying the collector
  and configuring other daemons.  The default is \Expr{True}

\label{param:SharedPortDefaultId}
\item[\Macro{SHARED\_PORT\_DEFAULT\_ID}] When \Macro{COLLECTOR\_USES\_SHARED\_PORT}
  is set to \Expr{True}, this is the shared port ID used by the \Condor{collector}.
  This defaults to \Expr{collector} and will not need to be changed by most sites.
...
\index{configuration macro!\texttt{SHARED\_PORT\_MAX\_FILE\_DESCRIPTORS}}
...
If the \Condor{shared\_port} daemon is in use, its value for this
...
\Procedure{gethostname}, which will be invoked by default
...
an HTCondor pool, via the \Attr{Machine} and \Attr{Name} attributes of 
daemon ClassAds.
...
\item[\Macro{PRIVATE\_NETWORK\_NAME}]
  ...
  (described below).
  The default is \MacroUNI{FULL\_HOSTNAME}.
...
\item[\Macro{UPDATE\_COLLECTOR\_WITH\_TCP}]
  ...
  send ClassAd updates to the \Condor{collector}.
  ...
  ensure that the \Condor{collector} has a large enough file descriptor limit
  using \Macro{COLLECTOR\_MAX\_FILE\_DESCRIPTORS}.
  The default value is \Expr{True}.
  % PKK
  % Described in default config file: YES
  % Defined in the default config file: NO
  % Default definition in config file: bogus
  % Result if not defined or RHS is empty: use TCP for updates to collectors
...
\item[\Macro{UPDATE\_VIEW\_COLLECTOR\_WITH\_TCP}]
  ...
  \Condor{collector} to forward ClassAd updates to the \Condor{collector}
  daemons specified by \Macro{CONDOR\_VIEW\_HOST}.
  Please read section~\ref{sec:tcp-collector-update} 
...
\item[\Macro{TCP\_UPDATE\_COLLECTORS}]
  The list of \Condor{collector} daemons which will be updated with 
  TCP instead of UDP
  when \MacroNI{UPDATE\_COLLECTOR\_WITH\_TCP} or
  \MacroNI{UPDATE\_VIEW\_COLLECTOR\_WITH\_TCP} is \Expr{False}.
  Please read section~\ref{sec:tcp-collector-update} 
...
\item[\Macro{UDP\_NETWORK\_FRAGMENT\_SIZE}]
	...
	is larger than \MacroUNI{UDP\_NETWORK\_FRAGMENT\_SIZE}, then the message
	will be split (fragmented) into multiple packets no larger than
	\MacroUNI{UDP\_NETWORK\_FRAGMENT\_SIZE}. If the destination of the message
	is the loopback network interface, see 
	\Macro{UDP\_LOOPBACK\_FRAGMENT\_SIZE} below. 
...
\item[\Macro{UDP\_LOOPBACK\_FRAGMENT\_SIZE}]
	...
	is larger than \MacroUNI{UDP\_LOOPBACK\_FRAGMENT\_SIZE}, then the message
	will be split (fragmented) into multiple packets no larger than
	\MacroUNI{UDP\_LOOPBACK\_FRAGMENT\_SIZE}.
	If the destination of the message is not the loopback interface,
	see \Macro{UDP\_NETWORK\_FRAGMENT\_SIZE} above.

\label{param:AlwaysReuseAddr}
\item[\Macro{ALWAYS\_REUSEADDR}]
  A boolean value that, when \Expr{True},
  tells HTCondor to set \MacroNI{SO\_REUSEADDR} socket option, so that
  the schedd can run large numbers of very short jobs without exhausting the
  number of local ports needed for shadows.
  The default value is \Expr{True}.  (Note that this represents a change
  in behavior compared to versions of HTCondor older than 8.6.0,
  which did not include this configuration macro.
  To restore the previous behavior, set this value to \Expr{False}.)
...
\item[\Macro{ALWAYS\_USE\_LOCAL\_CKPT\_SERVER}]
  A boolean value that defaults to \Expr{False}.
  When \Expr{True}, it forces all checkpoints to be read from a checkpoint
...
\label{param:SubsysArgs}
  ...
  \index{configuration macro!\texttt{NEGOTIATOR\_ARGS}}
  \index{configuration macro!\texttt{COLLECTOR\_ARGS}}
  \index{configuration macro!\texttt{STARTD\_ARGS}}
  \index{configuration macro!\texttt{SCHEDD\_ARGS}}
...
\item[\Macro{START\_MASTER}]
  If this setting is defined and set to \Expr{False} the \Condor{master}
  will immediately exit upon startup.
  This appears strange, but perhaps you
  ...
  set of files -- setting \MacroNI{START\_MASTER} to \Expr{False}
  for those machines would allow this.
  Note that \MacroNI{START\_MASTER} is
  an entry you would most likely find in a local configuration file,
  not a global configuration file.
  If not defined, \MacroNI{START\_MASTER} defaults to \Expr{True}.
...
\label{param:DefaultMasterShutdownScript}
\item[\Macro{DEFAULT\_MASTER\_SHUTDOWN\_SCRIPT}]
  ...
  This allows the admin to specify a program to execute as root
  when the \Condor{master} exits.  Note that a successful call
  to the \Condor{set\_shutdown} program will override this setting; see
  the documentation for config knob \Macro{MASTER\_SHUTDOWN\_<Name>} below.
...
\item[\Macro{WINDOWS\_FIREWALL\_FAILURE\_RETRY}]
  ...
  times (once every 5 seconds) that the \Condor{master} will retry
...
\item[\Macro{DISCARD\_SESSION\_KEYRING\_ON\_STARTUP}]
  ...
  the \Condor{master} daemon will replace the kernel session keyring
  it was invoked with with a new keyring named \Expr{htcondor}.  
  ...
  By replacing the keyring on start up, the \Condor{master} ensures
...
\item[\Macro{ENABLE\_KERNEL\_TUNING}]
  ...
  to \Expr{True}.
  When \Expr{True},
  the \Condor{master} daemon invokes the kernel tuning script 
  specified by configuration variable \MacroNI{LINUX\_KERNEL\_TUNING\_SCRIPT}
  once as root when the \Condor{master} daemon starts up.

\item[\Macro{KERNEL\_TUNING\_LOG}]
  A string value that defaults to \File{\$(LOG)/KernelTuningLog}.  
  If the kernel tuning script runs, its output will be logged to this file.

\item[\Macro{LINUX\_KERNEL\_TUNING\_SCRIPT}]
  A string value that defaults to \File{\$(LIBEXEC)/linux\_kernel\_tuning}.
  This is the script that the \Condor{master} runs to tune the
  kernel when \MacroNI{ENABLE\_KERNEL\_TUNING} is \Expr{True}.
...
\item[\Macro{ADVERTISE\_PSLOT\_ROLLUP\_INFORMATION}]
  A boolean value that defaults to \Expr{True},
  causing the \Condor{startd} to advertise ClassAd attributes that may be
  used in partitionable slot preemption.
  The attributes are
  \begin{itemize}
  \item \Attr{ChildAccountingGroup}
  \item \Attr{ChildActivity}
  \item \Attr{ChildCPUs}
  \item \Attr{ChildCurrentRank}
  \item \Attr{ChildEnteredCurrentState}
  \item \Attr{ChildMemory}
  \item \Attr{ChildName}
  \item \Attr{ChildRemoteOwner}
  \item \Attr{ChildRemoteUser}
  \item \Attr{ChildRetirementTimeRemaining}
  \item \Attr{ChildState}
  \item \Attr{PslotRollupInformation}
  \end{itemize}

\label{param:StartdPartitionableSlotAttrs}
\item[\Macro{STARTD\_PARTITIONABLE\_SLOT\_ATTRS}]
  A list of additional from the above default attributes from dynamic slots
  that will be rolled up into a list attribute in their parent partitionable 
  slot, prefixed with the name Child.
...
\item[\Macro{STARTD\_HISTORY}]
  ...
  This can be read with the \Condor{history} command by passing the
  name of the file to the -file option of \Condor{history}.
\begin{verbatim}
...
\item[\Macro{CLAIM\_WORKLIFE}]
  ...
  Once the \Condor{negotiator} gives a \Condor{schedd} a claim to a slot, 
...
\item[\Macro{STARTD\_JOB\_ATTRS}]
  ...
  This setting was formerly called \MacroNI{STARTD\_JOB\_EXPRS}.
...
\item[\Macro{NUM\_CPUS}]
  ...
  The default value is \MacroU{DETECTED\_CPUS}.

  The \Condor{startd} only takes note of the value of this configuration
...
\item[\Macro{COUNT\_HYPERTHREAD\_CPUS}]
  ...
  processors. When set to the default value of \Expr{True}, 
  it includes virtual CPUs in
  the default value of \MacroNI{DETECTED\_CPUS}. On dedicated cluster nodes, 
...
\item[\Macro{RUNBENCHMARKS}]
  ...
  If \MacroNI{RUNBENCHMARKS} is specified and set to anything other 
  than \Expr{False},
  additional benchmarks will be run once,
  when the \Condor{startd} starts.
...
\end{verbatim}
  ...
  If the job's execute directory is encrypted, \File{/tmp} and
  \File{/var/tmp} are automatically added to
  \MacroNI{MOUNT\_UNDER\_SCRATCH} when the job is run (they
  will not show up if \MacroNI{MOUNT\_UNDER\_SCRATCH} is
  examined with \Condor{config\_val}).

  \Bold{Note that the \MacroNI{MOUNT\_UNDER\_SCRATCH} mounts do not
  take place until the PreCmd of the job, if any, completes.}
  (See~\ref{man-condor-submit-prescript} for information on PreCmd.)

  Also note that, if \MacroNI{MOUNT\_UNDER\_SCRATCH} is defined, it
  must either be a string or an expression that evaluates to a
  string.

  For Docker Universe jobs, any directories that are mounted under scratch are
  also volume mounted on the same paths inside the container.  That is, any
  reads or writes to files in those directories goes to the host filesytem
  under the scratch directory.  This is useful if a container has limited space
  to grow a filesytem.

\end{description}

The following macros control if the \Condor{startd} daemon should perform
...
The following macros only apply to the \Condor{startd} daemon when it
is running on a multi-core machine. 
...
\item[\Macro{SLOTS\_CONNECTED\_TO\_CONSOLE}] 
  ...
  console.
  This allows the \Condor{startd} to notice console activity.
  Defaults to the number of slots in the machine,
  which is \MacroUNI{NUM\_CPUS}.
...
\item[\Macro{ENVIRONMENT\_FOR\_Assigned<name>}]
  ...
  defined by the slot ClassAd attribute \Attr{Assigned<name>}.
  ...
  if the environment variable name is \Env{CUDA\_VISIBLE\_DEVICES}, 
...
\item[\Macro{ENVIRONMENT\_VALUE\_FOR\_UnAssigned<name>}]
  ...
  in by configuration variable \MacroNI{ENVIRONMENT\_FOR\_Assigned<name>}
  when there is no machine ClassAd attribute \Attr{Assigned<name>} 
  for the slot. 
...
\end{verbatim}
  and there is \emph{no} machine ClassAd attribute \Attr{AssignedGPUs},
  the job's environment will contain
\begin{verbatim}
  VISIBLE_GPUS = none
\end{verbatim}
...
\item[\Macro{MODIFY\_REQUEST\_EXPR\_REQUESTMEMORY}]
  An integer expression used by the \Condor{startd} daemon to modify the
...
\item[\Macro{SLOT\_WEIGHT}]
  ...
  the \Condor{negotiator} daemon during matchmaking to assess user usage
...
\item[\Macro{DOCKER}]
  ...
  value is /usr/bin/docker.  Remember that the condor user must also
  be in the docker group for Docker Universe to work.
...
\label{param:OpenmpiInstallPath}
\item[\Macro{OPENMPI\_INSTALL\_PATH}]
  The location of the Open MPI installation on the local machine.
  Referenced by \File{examples/openmpiscript}, 
  which is used for running Open MPI jobs in the parallel universe.
  The Open MPI bin and lib directories should exist under this path.
  The default value is \File{/usr/lib64/openmpi}.

\label{param:OpenmpiExcludeNetworkInterfaces}
\item[\Macro{OPENMPI\_EXCLUDE\_NETWORK\_INTERFACES}]
  A comma-delimited list of network interfaces that Open MPI
  should not use for MPI communications.
  Referenced by \File{examples/openmpiscript}, 
  which is used for running Open MPI jobs in the parallel universe.

  The list should contain any interfaces that your job could
  potentially see from any execute machine.
  The list may contain undefined interfaces without generating errors.
  Open MPI should exclusively use low latency/high speed networks it finds
  (e.g. InfiniBand) regardless of this setting.
  The default value is \Attr{docker0},\Attr{virbr0}.
...
\item[\Macro{START\_LOCAL\_UNIVERSE}]
  ...
  The following example only allows 10 \SubmitCmdNI{local} universe jobs to
...
\item[\Macro{STARTER\_LOCAL}]
  ...
  run for \SubmitCmdNI{local} universe jobs.  This variable's value
  ...
  \SubmitCmdNI{local} universe to one that includes the
  \SubmitCmdNI{local} universe, but without utilizing the newer, provided
  configuration files.
...
\item[\Macro{START\_SCHEDULER\_UNIVERSE}]
  ...
  The \Condor{schedd} uses this macro to determine whether to start
  a \SubmitCmdNI{scheduler} universe job. 
  At intervals determined by \MacroNI{SCHEDD\_INTERVAL}, 
  the \Condor{schedd} daemon evaluates this macro
  for each idle \SubmitCmdNI{scheduler} universe job that it has.
  For each job, if the \MacroNI{START\_SCHEDULER\_UNIVERSE} 
  macro is \Expr{True}, then the job's \Macro{Requirements} expression
  is evaluated. If both conditions are met, then the job is allowed
  to begin execution. 
  
  The following example only allows 10 \SubmitCmdNI{scheduler} universe jobs to
...
\item[\Macro{MAX\_JOBS\_RUNNING}]
  An integer representing a limit on the number of \Condor{shadow} processes
...
\item[\Macro{MAX\_JOBS\_PER\_OWNER}]
  ...
  permitted to have within a \Condor{schedd} daemon's queue.  
 ...
 \MacroNI{MAX\_JOBS\_SUBMITTED},
  to ensure that no one user can dominate the queue.

\label{param:MaxRunningSchedulerJobsPerOwner}
\item[\Macro{MAX\_RUNNING\_SCHEDULER\_JOBS\_PER\_OWNER}]
  This integer value limits the number of scheduler universe jobs that any
  given owner (user) can have running at one time.  This limit will affect
  the number of running Dagman jobs, but not the number of nodes within a
  DAG. The default is no limit
...
\item[\Macro{MAX\_JOBS\_PER\_SUBMISSION}]
  ...
  is permitted to add to a \Condor{schedd} daemon's queue.  The
  ...
  and for protecting a busy \Condor{schedd} daemon
...
\item[\Macro{CURB\_MATCHMAKING}]
  A ClassAd expression evaluated by the \Condor{schedd} in the context of the
  \Condor{schedd} daemon's own ClassAd.  
  While this expression evaluates to \Expr{True}, the
  \Condor{schedd} will refrain from requesting more resources from a
  \Condor{negotiator}.  Defaults to \Expr{False}.
...
\item[\Macro{CONDOR\_Q\_USE\_V3\_PROTOCOL}]
  A boolean value that, when \Expr{True},
  causes the \Condor{schedd} to use an algorithm that responds to 
  \Condor{q} requests by \emph{not} forking itself to handle each request.
  It instead handles the requests in a non-blocking way.
  The default value is \Expr{True}.

\label{param:CondorQDashBatchIsDefault}
\item[\Macro{CONDOR\_Q\_DASH\_BATCH\_IS\_DEFAULT}]
  A boolean value that, when \Expr{True},
  causes \Condor{q} to print the \Opt{-batch} output unless the
  \Opt{-nobatch} option is used or the other arguments to \Condor{q} are
  incompatible with batch mode.  For instance \Opt{-long} is incompatible with \Opt{-batch}.
  The default value is \Expr{True}.

\label{param:CondorQOnlyMyJobs}
\item[\Macro{CONDOR\_Q\_ONLY\_MY\_JOBS}]
  A boolean value that, when \Expr{True},
  causes \Condor{q} to request that only the current user's jobs be queried
  unless the current user is a queue superuser.  It also causes the
  \Condor{schedd} to honor that request.
  The default value is \Expr{True}. A value of \Expr{False} in either \Condor{q} or
  the \Condor{schedd} will result in the old behavior of querying all jobs.

\label{param:CondorQShowOldSummary}
\item[\Macro{CONDOR\_Q\_SHOW\_OLD\_SUMMARY}]
  A boolean value that, when \Expr{True},
  causes \Condor{q} to show the old single line summary totals. When \Expr{False}
  \Condor{q} will show the new multi-line summary totals.
...
\label{param:AbsentSubmitterLifetime}
\item[\Macro{ABSENT\_SUBMITTER\_LIFETIME}]
  This macro determines the maximum time that the \Condor{schedd} will
  remember a submitter after the last job for that submitter leaves
  the queue. It is defined in terms of seconds and defaults to 1 week.

\label{param:AbsentSubmitterUpdateRate}
\item[\Macro{ABSENT\_SUBMITTER\_UPDATE\_RATE}]
  This macro can be used to set the maximum rate at which the \Condor{schedd}
  sends updates to the \Condor{collector} for submitters that have no
  jobs in the queue. It is defined in terms of seconds and defaults to 300
  (every 5 minutes).
...
\item[\Macro{JOB\_IS\_FINISHED\_COUNT}]
  An integer value representing the number of jobs that the \Condor{schedd}
...
\item[\Macro{JOB\_IS\_FINISHED\_INTERVAL}]
  ...
  The default value is 0, 
  which tells the \Condor{schedd} to not impose any delay.  
...
\item[\Macro{STARTD\_SENDS\_ALIVES}]
  ...
  old \Condor{schedd} daemons with newer pools.
  A boolean value that defaults to \Expr{True},
...
\item[\Macro{SYSTEM\_PERIODIC\_HOLD}]
  ...
  to the job expression \AdAttr{periodic\_hold}, but it is evaluated
...
\item[\Macro{SIGNIFICANT\_ATTRIBUTES}]
  ...
  When defined, this list replaces the list that the \Condor{negotiator} 
  would define based upon machine ClassAds.

\label{param:AddSignificantAttributes}
\item[\Macro{ADD\_SIGNIFICANT\_ATTRIBUTES}]
  A comma and/or space separated list of job ClassAd attributes that
  will always be added to the list of attributes that the \Condor{negotiator}
...
\item[\Macro{REMOVE\_SIGNIFICANT\_ATTRIBUTES}]
  A comma and/or space separated list of job ClassAd attributes that
  are removed from the list of attributes that the \Condor{negotiator}
  defines based upon machine ClassAds, for determining
  the sets of jobs considered as a unit (an auto cluster) in negotiation,
  when auto clustering is enabled.
...
\item[\Macro{SCHEDD\_USE\_SLOT\_WEIGHT}]
  A boolean that defaults to \Expr{False}. When \Expr{True}, 
  the \Condor{schedd} does use configuration variable \MacroNI{SLOT\_WEIGHT} 
  to weight running and idle job counts in the submitter ClassAd.

\label{param:JobTransformNames}
\item[\Macro{JOB\_TRANSFORM\_NAMES}]
  A comma and/or space separated list of unique names,
  where each is used in the formation
  of a configuration variable name that will contain a set
  of rules governing the transformation of jobs during submission.
  Each name in the list will be used in the name of
  configuration variable \MacroNI{JOB\_TRANSFORM\_<Name>}.
  Transforms are applied in the order in which names appear in this list.
  Names are not case-sensitive.
  There is no default value.

\label{param:JobTransformName}
\item[\Macro{JOB\_TRANSFORM\_<Name>}]
  A single job transform specified as a set of transform rules in new classad syntax.
  The transform rules are applied to jobs that match the transform's
  \Attr{Requirements} expression as they are submitted.
  \MacroNI{<Name>} corresponds to a name listed in
  \MacroNI{JOB\_TRANSFORM\_NAMES}.
  Names are not case-sensitive.
  There is no default value.
...
\item[\Macro{SUBMIT\_REQUIREMENT\_NAMES}]
  ...
  Each name in the list will be used in the name of
  configuration variable \MacroNI{SUBMIT\_REQUIREMENT\_<Name>}.
  There is no default value.

\label{param:SubmitRequirementName}
\item[\Macro{SUBMIT\_REQUIREMENT\_<Name>}]
  A boolean expression 
  evaluated in the context of the \Condor{schedd} daemon ClassAd,
  which is the \Expr{MY.} name space and the job ClassAd, 
  which is the \Expr{TARGET.} name space.
  When \Expr{False}, it causes the \Condor{schedd} to reject 
...
\item[\Macro{SUBMIT\_REQUIREMENT\_<Name>\_REASON}]
  ...
  to be printed for the job submitter when \MacroNI{SUBMIT\_REQUIREMENT\_<Name>}
  evaluates to \Expr{False} and the \Condor{schedd} rejects the job.
  There is no default value.
...
\item[\Macro{SCHEDD\_RESTART\_REPORT}]
  ...
  The report is written when the \Condor{schedd} starts.
  It contains statistics
  about its attempts to reconnect to the \Condor{startd} daemons for all jobs
  ...
  address specified by \MacroNI{CONDOR\_ADMIN}.
  The default value is \File{\$(LOG)/ScheddRestartReport}.
...
\item[\Macro{CHOWN\_JOB\_SPOOL\_FILES}]
  Prior to HTCondor 8.5.0 on unix, the \condor{schedd} would chown job
  files in the SPOOL directory between the condor account and the
  account of the job submitter.
  Now, these job files are always owned by the job submitter by default.
  To restore the older behavior, set this parameter to \Expr{True}.
  The default value is \Expr{False}.

\label{param:ImmutableJobAttrs}
\item[\Macro{IMMUTABLE\_JOB\_ATTRS}]
  A comma and/or space separated list of attributes provided by the
  administrator that cannot be changed, once they have committed values.
  No attributes are in this list by default.

\label{param:SystemImmutableJobAttrs}
\item[\Macro{SYSTEM\_IMMUTABLE\_JOB\_ATTRS}]
  A predefined comma and/or space separated list of attributes that
  cannot be changed, once they have committed values.
  The hard-coded value is:
  \Attr{Owner} \Attr{ClusterId} \Attr{ProcId} \Attr{MyType} \Attr{TargetType}.

\label{param:ProtectedJobAttrs}
\item[\Macro{PROTECTED\_JOB\_ATTRS}]
  A comma and/or space separated list of attributes provided by the
  administrator that can only be altered by the queue super-user,
  once they have committed values.
  No attributes are in this list by default.

\label{param:SystemProtectedJobAttrs}
\item[\Macro{SYSTEM\_PROTECTED\_JOB\_ATTRS}]
  A predefined comma and/or space separated list of attributes that can only
  be altered by the queue super-user, once they have committed values.
  The hard-code value is empty.
...
\label{param:ShadowStatsLog}
\item[\Macro{SHADOW\_STATS\_LOG}]
  The full path and file name of a file that stores TCP statistics
  for shadow file transfers.  (Note that the shadow logs TCP statistics
  to this file by default.  Adding \MacroNI{D\_STATS} to the
  \MacroNI{SHADOW\_DEBUG} value will cause TCP statistics to be logged
  to the normal shadow log file (\MacroUNI{SHADOW\_LOG}).)
  If not defined, \MacroNI{SHADOW\_STATS\_LOG}
  defaults to \Expr{\$(LOG)/XferStatsLog}.
  Setting \MacroNI{SHADOW\_STATS\_LOG} to \Expr{/dev/null} disables logging
  of shadow TCP file transfer statistics.

\label{param:MaxShadowStatsLog}
\item[\Macro{MAX\_SHADOW\_STATS\_LOG}]
  ...
  \MacroNI{MAX\_SHADOW\_STATS\_LOG} defaults to \MacroUNI{MAX\_DEFAULT\_LOG},
  which currently defaults to 10 MiB in size.
  Values are specified with the same syntax as \MacroNI{MAX\_DEFAULT\_LOG}.
...
\item[\Macro{STARTER\_LOG\_NAME\_APPEND}]
  ...
  by the \Condor{starter} daemon. 
  Permitted values are \Expr{true}, \Expr{false}, \Expr{slot}, 
  \Expr{cluster} and \Expr{jobid}.
  A value of \Expr{false} will suppress the use of a file extension.
  A value of \Expr{true} gives the default behavior of using the slot name,
  unless there is only a single slot. 
  A value of \Expr{slot} uses the slot name.
  A value of \Expr{cluster} uses the job's \Attr{ClusterId} ClassAd attribute.
  A value of \Expr{jobid} uses the job's \Attr{ProcId} ClassAd attribute.
  If \Expr{cluster} or \Expr{jobid} are specified,
...
\item[\Macro{CGROUP\_MEMORY\_LIMIT\_POLICY}]
  A string with possible values of \Expr{hard}, \Expr{soft} and \Expr{none}.
  The default value is \Expr{none}.
  ...
  If set to the default value \Expr{soft}, 
  ...
  If set to \Expr{none}, no limit will be enforced, but the memory
  usage of the job will be accurately measured by a cgroup.
...
\item[\Macro{ASSIGN\_CPU\_AFFINITY}]
  A boolean expression that defaults to \Expr{False}.
  When it evaluates to \Expr{True},
  each job under this \Condor{startd} is confined to using only as many
  ...
  specifying \SubmitCmdNI{request\_cpus}.
  When \Expr{True}, this configuration variable overrides any specification
  of \MacroNI{ENFORCE\_CPU\_AFFINITY}. The expression is evaluated in the
  context of the Job ClassAd.

\label{param:EnforceCpuAffinity} 
\item[\Macro{ENFORCE\_CPU\_AFFINITY}]
  This configuration variable is replaced by \MacroNI{ASSIGN\_CPU\_AFFINITY}.
  ...
  When \Expr{True}, the processes in an HTCondor job maintain their
  affinity to a CPU.  This means that this job will only run on
  that particular CPU, even if other CPU cores are idle.

  If \Expr{True} and \MacroNI{SLOT<N>\_CPU\_AFFINITY} is not set,
  the CPU that the job is locked to is the same as \Expr{SlotID - 1}.
...
\item[\Macro{RUN\_FILETRANSFER\_PLUGINS\_WITH\_ROOT}]
  A boolean value that affects only Unix platforms and defaults to \Expr{False},
  ...
  The user that the job runs as may be the job owner, \Login{nobody},
  or the slot user.
  ...
  file transfer plug-ins are invoked with a real UID of 0 (\Login{root}),
  provided the HTCondor daemons also run as \Login{root}.
...
\item[\Macro{ENABLE\_CHIRP\_DELAYED}]
  A boolean value that defaults to \Expr{True}. 
  If \Expr{False}, the \Condor{chirp} commands
  \Opt{get\_job\_attr\_delayed} and \Opt{set\_job\_attr\_delayed} are prohibited.

\label{param:ChirpDelayedUpdatePrefix}
\item[\Macro{CHIRP\_DELAYED\_UPDATE\_PREFIX}]
  This string-valued parameter, which defaults to \AdStr{Chirp*},
  defines the allowed prefixes for attribute names which can be used with the
  \Condor{chirp} commands \Opt{set\_job\_attribute\_delayed} and 
  \Opt{get\_job\_attribute\_delayed}.  Because it must be set to the
  same value on both the submit and execute nodes, it is advised that this
  parameter not be changed from its built-in default.

\label{param:ChirpDelayedUpdateMaxAttrs}
\item[\Macro{CHIRP\_DELAYED\_UPDATE\_MAX\_ATTRS}]
  This integer-valued parameter, which defaults to 100, represents the maximum
  number of pending delayed chirp updates buffered by the \Condor{starter}.
  If the number of unique attributes updated by the \Condor{chirp} command
  \Opt{set\_job\_attr\_delayed} exceeds this parameter, it is possible for
...
\item[\Macro{DYNAMIC\_RUN\_ACCOUNT\_LOCAL\_GROUP}]
  ...
  than the default \Expr{Users} for the \Expr{condor-slot<X>} run account.
...
\label{param:StarterStatsLog}
\item[\Macro{STARTER\_STATS\_LOG}]
  The full path and file name of a file that stores TCP statistics
  for starter file transfers.  (Note that the starter logs TCP statistics
  to this file by default.  Adding \MacroNI{D\_STATS} to the
  \MacroNI{STARTER\_DEBUG} value will cause TCP statistics to be logged
  to the normal starter log file (\MacroUNI{STARTER\_LOG}).)
  If not defined, \MacroNI{STARTER\_STATS\_LOG}
  defaults to \Expr{\$(LOG)/XferStatsLog}.
  Setting \MacroNI{STARTER\_STATS\_LOG} to \Expr{/dev/null} disables
  logging of starter TCP file transfer statistics.

\label{param:MaxStarterStatsLog}
\item[\Macro{MAX\_STARTER\_STATS\_LOG}]
  Controls the maximum size in bytes or amount of time that the starter
  TCP statistics log will be allowed to grow.  If not defined,
  \MacroNI{MAX\_STARTER\_STATS\_LOG} defaults to \MacroUNI{MAX\_DEFAULT\_LOG},
  which currently defaults to 10 MiB in size.
  Values are specified with the same syntax as \MacroNI{MAX\_DEFAULT\_LOG}.

\label{param:Singularity}
\item[\Macro{SINGULARITY}]
  The path to the Singularity binary.
  The default value is \File{/usr/bin/singularity}.

\label{param:SingularityJob}
\item[\Macro{SINGULARITY\_JOB}]
  A boolean value specifying whether this startd should run jobs
  under Singularity.
  The default value is \Expr{False}.

\label{param:SingularityImageExpr}
\item[\Macro{SINGULARITY\_IMAGE\_EXPR}]
  The path to the Singularity container image file.
  The default value is \AdStr{SingularityImage}.

\label{param:SingularityTargetDir}
\item[\Macro{SINGULARITY\_TARGET\_DIR}]
  A directory within the Singularity image to which
  \MacroNI{\$\_CONDOR\_SCRATCH\_DIR}
  on the host should be mapped.
  The default value is \AdStr{}.

\label{param:SingularityBindExpr}
\item[\Macro{SINGULARITY\_BIND\_EXPR}]
  A string value containing a list of bind mount specifications to
  be passed to Singularity.
  The default value is \AdStr{SingularityBind}.
...
\label{param:DefaultJobMaxRetries}
\item[\Macro{DEFAULT\_JOB\_MAX\_RETRIES}]
  The default value for the maximum number of job retries, if the
  \Condor{submit} retry feature is used.  (Note that this value is
  only relevant if either \SubmitCmdNI{retry\_until} or
  \SubmitCmdNI{success\_exit\_code} is defined in the submit file,
  and \SubmitCmdNI{max\_retries} is not.)
  (See section~\ref{condor-submit-max-retries} for more information.)
  The default value if not defined is 10.
...
\item[\Macro{SUBMIT\_ATTRS}]
  ...
  \Condor{submit},
  while the use of \MacroNI{SUBMIT\_ATTRS} adds a job ClassAd attribute
  at a later point in time.
  \MacroNI{SUBMIT\_EXPRS} is a historic setting that functions identically to
  \MacroNI{SUBMIT\_ATTRS}. It may be removed in the future, so use \MacroNI{SUBMIT\_ATTRS}.
...
\item[\Macro{CLASSAD\_LIFETIME}]
  ...
  To ensure that the \Condor{collector} does not miss any ClassAds, the
  ...
  \begin{itemize}
  \item \MacroNI{UPDATE\_INTERVAL} (for the \Condor{startd} daemon)
  \item \MacroNI{NEGOTIATOR\_UPDATE\_INTERVAL}
  \item \MacroNI{SCHEDD\_INTERVAL}
  \item \MacroNI{MASTER\_UPDATE\_INTERVAL}
  \item \MacroNI{CKPT\_SERVER\_INTERVAL}
  \item \MacroNI{DEFRAG\_UPDATE\_INTERVAL}
  \item \MacroNI{HAD\_UPDATE\_INTERVAL}
  \end{itemize}
...
\item[\Macro{COLLECTOR\_QUERY\_WORKERS}]
  ...
  number of child worker processes that the \Condor{collector} can have, and
  defaults to a value of 4 on Linux and MacOS platforms. 
  When receiving a large query request, 
  the  \Condor{collector} may \Procedure{fork} a new
  process to handle the query, freeing the main process to handle
  other requests.  Each forked child process will consume memory, potentially
  up to 50\% or more of the memory consumed by the parent collector process.
  To limit the amount of memory consumed on the central manager to 
  handle incoming queries, the default value for this macro is 4.
  When the number of outstanding worker processes
  reaches the maximum specified by this macro, 
  any additional incoming query requests will be queued and serviced
  after an existing child worker completes. 
  Note that on Windows platforms, this macro has a value of zero and cannot
  be changed.

\label{param:CollectorQueryWorkersReserveForHighPrio}
\item[\Macro{COLLECTOR\_QUERY\_WORKERS\_RESERVE\_FOR\_HIGH\_PRIO}]
  This macro defines the number of \Macro{COLLECTOR\_QUERY\_WORKERS} slots will
  be held in reserve to only service high priority query requests.  Currently,
  high priority queries are defined as those coming from the
  \Condor{negotiator} during the course of matchmaking, or via a ``condor\_sos
  condor\_status'' command.  The idea here is the critical operation of
  matchmaking machines to jobs will take precedence over user condor\_status
  invocations.  Defaults to a value of 1.  The maximum allowable value for this
  macro is equal to \Macro{COLLECTOR\_QUERY\_WORKERS} minus 1.

\label{param:CollectorQueryWorkersPending}
\item[\Macro{COLLECTOR\_QUERY\_WORKERS\_PENDING}]
  This macro sets the maximum of collector pending query requests that can
  be queued waiting for child workers to exit.  Queries that would exceed
  this maximum are immediately aborted.  When a forked child worker exits,
  a pending query will be pulled from the queue for service.
  Note the collector will confirm that the client has not closed the TCP socket
  (because it was tired of waiting) before going through all the work of
  actually forking a child and starting to service the query.  Defaults to a
  value of 50.

\label{param:CollectorQueryMaxWorktime}
\item[\Macro{COLLECTOR\_QUERY\_MAX\_WORKTIME}]
  This macro defines the maximum amount of time in seconds that a query has to
  complete before it is aborted.  Queries that wait in the pending queue longer
  than this period of time will be aborted before forking. Queries that have
  already forked will also abort after the worktime has expired - this protects
  against clients on a very slow network connection. If set to 0, then there is
  no timeout. The default is 0.

\label{param:HandleQueryInProcPolicy}
\item[\Macro{HANDLE\_QUERY\_IN\_PROC\_POLICY}]
  This variable sets the policy for which queries the \Condor{collector} should handle
  in process rather than by forking a worker. It should be set to one of the following values
  \begin{itemize}
  \item \MacroNI{always} Handle all queries in process
  \item \MacroNI{never}  Handle all queries using fork workers
  \item \MacroNI{small\_table} Handle only queries of small tables in process
  \item \MacroNI{small\_query} Handle only small queries in process
  \item \MacroNI{small\_table\_and\_query} Handle only small queries on small tables in process
  \item \MacroNI{small\_table\_or\_query} Handle small queries or small tables in process
  \end{itemize}
  A small table is any table of ClassAds in the collector other than Master,Startd,Generic and Any ads.
  A small query is a locate query, or any query with both a projection and a result limit that is smaller than 10.
  The default value is \MacroNI{small\_table\_or\_query}.
...
\label{param:CollectorForwardFiltering}
\item[\Macro{COLLECTOR\_FORWARD\_FILTERING}]
  When this boolean variable is set to \Expr{True}, Machine and
  Submitter ad updates are not forwarded to the
  \MacroNI{CONDOR\_VIEW\_HOST} if certain attributes are unchanged
  from the previous update of the ad.
  The default is \Expr{False}, meaning all updates are forwarded.

\label{param:CollectorForwardWatchList}
\item[\Macro{COLLECTOR\_FORWARD\_WATCH\_LIST}]
  When \MacroNI{COLLECTOR\_FORWARD\_FILTERING} is set to \Expr{True},
  this variable provides the list of attributes that controls whether
  a Machine or Submitter ad update is forwarded to the
  \MacroNI{CONDOR\_VIEW\_HOST}.
  If all attributes in this list are unchanged from the previous update,
  then the new update is not forwarded.
  The default value is \Expr{State,Cpus,Memory,IdleJobs}.

\label{param:CollectorForwardInterval}
\item[\Macro{COLLECTOR\_FORWARD\_INTERVAL}]
  When \MacroNI{COLLECTOR\_FORWARD\_FILTERING} is set to \Expr{True},
  this variable limits how long forwarding of updates for a given
  ad can be filtered before an update must be forwarded.
  The default is one third of \MacroNI{CLASSAD\_LIFETIME}.
...
\label{param:CollectorPersistentAdLog}
...
\item[\Macro{NEGOTIATOR\_UPDATE\_INTERVAL}]
  This macro determines how often
  the \Condor{negotiator} daemon sends a ClassAd update to the
  \Condor{collector}.  It is defined in seconds and defaults to 300
  (every 5 minutes).
...
\item[\Macro{NEGOTIATOR\_SOCKET\_CACHE\_SIZE}]
  ...
  currently 500.  To be effective, this parameter should be set to a
...
\item[\Macro{NEGOTIATOR\_INFORM\_STARTD}]
  ...
  The default is \Expr{False}.
  When this is set to the default value of \Expr{False}, 
  the \Condor{startd} will never
  ...
  the \Condor{negotiator} should be configured not to attempt to contact
  these \Condor{startd} daemons by using the default value.
...
\item[\Macro{NEGOTIATOR\_MAX\_TIME\_PER\_SCHEDD}]
  The maximum number of seconds the \Condor{negotiator}
  will spend with each individual \Condor{schedd} during one
  negotiation cycle.  Once this time limit has been reached, the
  \Condor{negotiator} will skip
  over requests from this \Condor{schedd} until the next negotiation cycle.
  It defaults to the number of seconds in one year.
...
\item[\Macro{ALLOW\_PSLOT\_PREEMPTION}]
  A boolean value that defaults to \Expr{False}.
  When set to \Expr{True} for the \Condor{negotiator},
...
\item[\Macro{GROUP\_SORT\_EXPR}]
  ...
  goes last when considering group quotas, and groups are
  considered in starvation order (the group using the smallest fraction
  of its resource quota is considered first).
...
\item[\Macro{MAX\_PROCD\_LOG}]
  ...
  file may grow without bounds.  The default is 10 MiB.
...
\item[\Macro{PROCD\_ADDRESS}]
  ...
  setting therefore depends on the platform and distribution: 
  \verb@$(LOCK)/procd_pipe@ or \verb@$(RUN)/procd_pipe@ on Unix and
...
\item[\Macro{BASE\_CGROUP}]
  ...
  This variable defaults to the string \Expr{htcondor}, and is only used on
  Linux systems.  To disable cgroup tracking, define this to an empty
  string.
...
\label{param:CreddPollingTimeout}
\item[\Macro{CREDD\_POLLING\_TIMEOUT}]
  An integer value that determines how long the \Condor{credd} daemon will
  poll for credentials in seconds. The default value is 20.
...
\item[\Macro{GRIDMANAGER\_MAX\_JOBMANAGERS\_PER\_RESOURCE}]
  For grid jobs of type \SubmitCmdNI{gt2}, limits the number of globus-job-manager
...
\item[\Macro{GRIDMANAGER\_GAHP\_RESPONSE\_TIMEOUT}]
  The \Condor{gridmanager} will assume a GAHP is hung if this many
...
\label{param:EC2GahpRateLimit}
\item[\Macro{EC2\_GAHP\_RATE\_LIMIT}]
  The minimum interval, in whole milliseconds, between requests to the same
  EC2 service with the same credentials.  Defaults to 100.
...
\item[\Macro{BOINC\_GAHP}]
  ...
  The default value is \File{\MacroUNI{SBIN}/boinc\_gahp}.
...
\label{param:JobRouterRoundRobinSelection}
\item[\Macro{JOB\_ROUTER\_ROUND\_ROBIN\_SELECTION}]
  A boolean value that controls which route is chosen for a candidate
  job that matches multiple routes.
  When set to \Expr{False}, the default, the first matching route is
  awlays selected.
  When set to \Expr{True}, the Job Router attempts to distribute jobs
  across all matching routes, round robin style.
...
\item[\Macro{ENABLE\_GRID\_MONITOR}]
  ...
  This parameter only affects grid jobs of type \SubmitCmdNI{gt2}.
...
Also note that configuration settings of a running \Condor{dagman} job are
not changed by doing a \Condor{reconfig}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:General}General}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{DAGMAN\_CONFIG\_FILE}]
  The path and name of the configuration file to be used by \Condor{dagman}.
  This configuration variable is set automatically by
  \Condor{submit\_dag}, and it should not be explicitly set by the user.
...
\item[\Macro{DAGMAN\_STARTUP\_CYCLE\_DETECT}]
  ...
  in addition to its run time cycle detection.  Note that setting
  this value to \Expr{True} will impose significant startup delays
  for large DAGs.
...
\item[\Macro{DAGMAN\_ABORT\_DUPLICATES}]
  ...
  \Expr{True}.
  \Bold{Note: users should rarely change this setting.}
...
\item[\Macro{DAGMAN\_USE\_OLD\_DAG\_READER}]
  ...
  Its value will always be \Expr{False}.  
  A setting of \Expr{True} will result in a warning, 
  ...
  \Bold{Note: users should never change this setting.}

\label{param:DAGmanUseSharedPort}
\item[\Macro{DAGMAN\_USE\_SHARED\_PORT}]
  A boolean value that controls whether \Condor{dagman} will attempt to
  connect to the shared port daemon.
  If not defined, \MacroNI{DAGMAN\_USE\_SHARED\_PORT} defaults to
  \Expr{False}.  There is no reason to ever change this value; it
  was introduced to prevent spurious shared port-related error messages
  from appearing in \File{dagman.out} files.
  (Introduced in version 8.6.1.)

\end{description}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Throttling}Throttling}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{DAGMAN\_MAX\_JOBS\_IDLE}]
  ...
  submitting jobs.  \Condor{dagman} will
  ...
  \MacroNI{DAGMAN\_MAX\_JOBS\_SUBMITTED}.
  Note that submit description files that queue multiple procs can
  cause the \MacroNI{DAGMAN\_MAX\_JOBS\_IDLE} limit to be exceeded.
  If a submit description file contains \Expr{queue 5000}
  and \MacroNI{DAGMAN\_MAX\_JOBS\_IDLE} is set to 250, 
  this will result in 5000 procs being submitted to the \Condor{schedd},
  ...
  to 0.
  This configuration option can be overridden by the \Condor{submit\_dag}
  \OptArg{-maxidle} command-line argument (see ~\ref{man-condor-submit-dag}).
...
\item[\Macro{DAGMAN\_MAX\_JOBS\_SUBMITTED}]
  ...
  A single invocation of \Condor{submit} by \Condor{dagman}
  counts as one job, even if the submit file produces a multi-proc cluster.
  The default value is 0 (unlimited).
  This configuration option can be overridden by the \Condor{submit\_dag}
  \OptArg{-maxjobs} command-line argument (see ~\ref{man-condor-submit-dag}).
...
\item[\Macro{DAGMAN\_MAX\_PRE\_SCRIPTS}]
  ...
  \Condor{dagman} will run at the same time.  
  The value 0 allows any number of PRE scripts to run.
  The default value if not defined is 20.
  Note that the \MacroNI{DAGMAN\_MAX\_PRE\_SCRIPTS} value can be overridden
  by the \Condor{submit\_dag} \Opt{-maxpre} command line option.
...
\item[\Macro{DAGMAN\_MAX\_POST\_SCRIPTS}]
  ...
  The default value if not defined is 20.
  Note that the \MacroNI{DAGMAN\_MAX\_POST\_SCRIPTS} value can be overridden
  by the \Condor{submit\_dag} \Opt{-maxpost} command line option.

\end{description}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Priority, node semantics}Priority, node semantics}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Node job submission/removal}Node job submission/removal}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{DAGMAN\_USER\_LOG\_SCAN\_INTERVAL}]
  ...
  \Condor{dagman} waits between checking the workflow log file for
  status updates.
  ...
  \Macro{DAGMAN\_MAX\_JOBS\_IDLE} is set to a small value.  If so,
  this will be noted in the \File{dagman.out} file.)
...
\item[\Macro{DAGMAN\_MAX\_SUBMITS\_PER\_INTERVAL}]
  ...
  \Bold{Note:  The maximum rate at which DAGMan can submit jobs is
  \MacroNI{DAGMAN\_MAX\_SUBMITS\_PER\_INTERVAL} /
  \MacroNI{DAGMAN\_USER\_LOG\_SCAN\_INTERVAL}.}
...
\label{param:DAGManPropagateHaltsToSubdags}
\item[\Macro{DAGMAN\_PROPAGATE\_HALTS\_TO\_SUBDAGS}]
  A boolean value that indicates whether \Condor{dagman}, upon noticing a halt
  file, will then propagate halt files to all nested DAGs causing them to halt
  also.
  If set to \Expr{False}, then nested DAGs will continue to submit jobs 
  normally despite the top-level DAG being halted.
  The default value if not defined is \Expr{True}.
...
\item[\Macro{DAGMAN\_SUPPRESS\_JOB\_LOGS}]
  ...
  The default value is \Expr{False}, such that events are written to
...
\item[\Macro{DAGMAN\_STORK\_SUBMIT\_EXE}]
  This configuration variable is no longer used; 
  as of HTCondor version 8.3.4, \Condor{dagman} no longer supports Stork jobs.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Rescue/retry}Rescue/retry}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Log files}Log files}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  in which \Bold{\Expr{@}} instead of \Bold{\Expr{\$}} indicates
  ...
  \item \Expr{@(OWNER)}: The user name of the user who submitted the
  DAG.
  \item \Expr{@(NODE\_NAME)}: For SUBDAGs, this is the node name of the SUBDAG
  in the upper level DAG; for a top-level DAG, it is the string \Expr{"undef"}.
  \end{itemize}
  ...
  such as \Expr{\$(LOCAL\_DIR)/log} (see examples below).
  ...
  \item \Bold{The value defined for \MacroNI{DAGMAN\_DEFAULT\_NODE\_LOG} must
...
\item[\Macro{DAGMAN\_LOG\_ON\_NFS\_IS\_ERROR}]
  A boolean value that controls whether \Condor{dagman} prohibits
  a DAG workflow log from being on an NFS file system.
  This value is ignored if \MacroNI{CREATE\_LOCKS\_ON\_LOCAL\_DISK} 
  and \MacroNI{ENABLE\_USERLOG\_LOCKING} are both \Expr{True}.
  If a DAG uses such a workflow log file file and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Debug output}Debug output}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{MAX\_DAGMAN\_LOG}]
  ...
  \MacroNI{MAX\_<SUBSYS>\_LOG}.
  If not defined, \MacroNI{MAX\_DAGMAN\_LOG} defaults to 0 (unlimited
  size).

\end{description}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:HTCondor attributes}HTCondor attributes}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{Metrics}Metrics}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{ENCRYPT\_EXECUTE\_DIRECTORY}]
  ...
  encrypted.
  Defaults to \Expr{False}.  
  Note that even if \Expr{False}, 
  the user can require encryption of the execute directory on a per-job 
  basis by setting \SubmitCmd{encrypt\_execute\_directory} to \Expr{True} 
  ...
  On Linux platforms, the encryption method is \Prog{ecryptfs},
  and therefore requires an installation of the \File{ecryptfs-utils} package.
...
\item[\Macro{ENCRYPT\_EXECUTE\_DIRECTORY\_FILENAMES}]
  ...
  (see the \Prog{ecryptfs} documentation).  
  ...
  by setting this configuration variable to \Expr{True}.

\label{param:EcryptfsAddPassphrase}
\item[\Macro{ECRYPTFS\_ADD\_PASSPHRASE}]
  The path to the \Prog{ecryptfs-add-passphrase} command-line utility. 
  If the path is not fully-qualified, 
  then safe system path subdirectories such as \File{/bin} and
  \File{/usr/bin} will be searched.  
  The default value is \File{ecryptfs-add-passphrase},
  ...
  sets \SubmitCmd{encrypt\_execute\_directory} to \Expr{True} in the
...
\label{param:CertificateMapfileAssumeHashKeys}
\item[\Macro{CERTIFICATE\_MAPFILE\_ASSUME\_HASH\_KEYS}]
  For HTCondor version 8.5.8 and later. When this is true, the second field of
  the \Macro{CERTIFICATE\_MAPFILE} is not interpreted as a regular expression
  unless it begins and ends with the slash / character.
...
\item[\Macro{SEC\_ENABLE\_MATCH\_PASSWORD\_AUTHENTICATION}]
  ...
  The default value is \Expr{True}.  
  To have any effect, it must be \Expr{True} in the configuration of both
...
These macros affect how HTCondor runs \SubmitCmdNI{vm} universe jobs on
...
These two variables have nothing to do with the \SubmitCmdNI{vm}
universe.
...
For general discussion of the \Condor{shared\_port} daemon,
...
\item[\Macro{USE\_SHARED\_PORT}] A boolean value that
  ...
  \Condor{shared\_port} daemon for receiving incoming connections.  
...
\item[\Macro{SHARED\_PORT\_PORT}]
  The default TCP port used by the \Condor{shared\_port} daemon.
  If \MacroNI{COLLECTOR\_USES\_SHARED\_PORT} is the default value 
  of \Expr{True},
  and the \Condor{master} launches a \Condor{collector} daemon,
  then the \Condor{shared\_port} daemon will ignore this value 
  and use the TCP port assigned to the \Condor{collector} 
  via the \MacroNI{COLLECTOR\_HOST} configuration variable.

  The default value is \MacroUNI{COLLECTOR\_PORT}, which defaults to 9618.  
  Note that this causes all HTCondor hosts to use TCP port 9618 by default, 
  differing from previous behavior.
  The previous behavior has only the \Condor{collector} host using a fixed port.
  To restore this previous behavior, 
  set \MacroNI{SHARED\_PORT\_PORT} to \Expr{0},
  which will cause the \Condor{shared\_port} daemon
  to use a randomly selected port in the range
  \MacroNI{LOWPORT} - \MacroNI{HIGHPORT},
...
\item[\Macro{DAEMON\_SOCKET\_DIR}] This specifies the directory where
  ...
  which is the port assigned to \Condor{shared\_port}.
  In this case, commands that interact with the execute node, 
  such as \Condor{ssh\_to\_job},
  ...
  The \Condor{master} picks a random prefix for abstract socket names
  and shares it privately with the other daemons.
  When searching for the recipient of an incoming connection,
  \Condor{shared\_port} will check for both an abstract socket and a
  named socket in the directory indicated by this variable.
  The named socket allows command-line tools such as \Condor{ssh\_to\_job} to
  use \Condor{shared\_port} as described.

  On Linux platforms,
  setting \Macro{SHARED\_PORT\_AUDIT\_LOG} causes HTCondor to log the
  following information about each connection made through the
  \MacroNI{DAEMON\_SOCKET\_DIR}: the source address, the socket file
  ...
  The default value is \Expr{auto},
  causing the use of the directory \Expr{\$(LOCK)/daemon\_sock}.
  On Unix platforms other than Linux, 
  if that path is longer than the 90 characters maximum,
  then the \Condor{master} will instead create a
  directory under \Expr{/tmp} with a name that looks like
  \Expr{/tmp/condor\_shared\_port\_<XXXXXX>},
  where \Expr{<XXXXXX>} is replaced with random characters.
  The \Condor{master} then tells the other daemons the exact name of
  the directory it created, and they use it.

  If a different value is set for \MacroNI{DAEMON\_SOCKET\_DIR},
...
\item[\Macro{SHARED\_PORT\_ARGS}] Like all daemons started by the
  \Condor{master} daemon, the command line arguments to the invocation
  of the \Condor{shared\_port} daemon can be customized.
  The arguments can be used to specify a non-default port number for the
  \Condor{shared\_port} daemon as in this example, which specifies port 4080:

\begin{verbatim}
  SHARED_PORT_ARGS = -p 4080
\end{verbatim}
  It is recommended to use configuration variable 
  \MacroNI{SHARED\_PORT\_PORT} to set a non-default port number,
  instead of using this configuration variable.

\label{param:SharedPortAuditLog}
\item[\Macro{SHARED\_PORT\_AUDIT\_LOG}]
  On Linux platforms,
  the path and file name of the \Condor{shared\_port} log that records
  connections made via the \MacroNI{DAEMON\_SOCKET\_DIR}.
  If not defined, there will be no \Condor{shared\_port} audit log.

\label{param:MaxSharedPortAuditLog}
\item[\Macro{MAX\_SHARED\_PORT\_AUDIT\_LOG}]
  On Linux platforms,
  controls the maximum amount of time that the \Condor{shared\_port}
  ...
  The \Expr{.old} files are overwritten each time the maximum number of
  rotated files
  (determined by the value of \MacroNI{MAX\_NUM\_SHARED\_PORT\_AUDIT\_LOG})
  is exceeded.
  A value of 0 specifies that the file may grow without bounds.
  The following suffixes may be used to qualify the integer:
\begin{description}
...
\item[\Macro{MAX\_NUM\_SHARED\_PORT\_AUDIT\_LOG}]
  On Linux platforms,
  the integer that controls the maximum number of rotations that
  the \Condor{shared\_port} audit log is allowed to perform,
...
\label{param:DefragUpdateInterval}
\item[\Macro{DEFRAG\_UPDATE\_INTERVAL}] An integer that specifies
  the number of seconds between times that the \Condor{defrag} daemon
  sends updates to the collector.  (See 
  section~\ref{sec:Defrag-ClassAd-Attributes} for information about the
  attributes in these updates.)
...
\item[\Macro{GANGLIAD\_VERBOSITY}] An integer that specifies the
  ...
  contains \Expr{TRANSFER:2}, or if the \MacroNI{STATISTICS\_TO\_PUBLISH\_LIST}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Config-annex}\Condor{annex} Configuration File Macros}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{configuration!condor\_annex configuration variables}

See section \ref{sec:clouds-config} for \Condor{annex} configuration file
macros.

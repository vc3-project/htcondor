...
\Bold{Note that a \Condor{hold} is not propagated to sub-DAGs.} 
In other words, if you \Condor{hold} a DAG that has sub-DAGs, any sub-DAGs that 
are already in the queue will continue to submit node jobs.

\Bold{A DAG halt, however, \emph{does} propagate by default to sub-DAGs.} 
If you create a halt file for a DAG, \Condor{dagman} will automatically generate 
halt files for all sub-DAGs. You can prevent this behavior by setting the
\MacroNI{PROPAGATE\_HALTS\_TO\_SUBDAGS} configuration variable to False.

Both \Condor{hold} or DAG halt apply to splices, because they are merged into 
the parent DAG and controlled by a single \Condor{dagman} instance.

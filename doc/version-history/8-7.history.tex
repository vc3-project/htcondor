...
\item In DAGMan, the appearance of a .halt file would previously have no effect
on SubDAGs. These would continue to submit jobs normally despite the top-level
DAG being halted. Now .halt files are automatically propagated to all SubDAGs. 
A new knob, DAGMAN\_PROPAGATE\_HALTS\_TO\_SUBDAGS, can be set to False to
